...
exports.listen = function (options, transportUtil) {
  return function (msg, callback) {
    ...
    server.on('request', function (req, res) {
      ...
      internals.setBody(listenOptions, seneca, transportUtil, req, res, function (err) {
        if (err) {
          return res.end()
        }
  ...
  return function (msg, callback) {
    ...
    var send = function (spec, topic, send_done) {
      ...
      function action (msg, done, meta) {
        ...
        if (clientOptions.secret) {
          headers['seneca-secret'] = clientOptions.secret
        }
...
internals.setBody = function (listenOptions, seneca, transportUtil, req, res, next) {
  var buf = []
  ...
  req.on('end', function () {
    try {
      ...
      if (req.headers['seneca-secret'] && req.headers['seneca-secret'] !== listenOptions.secret) {
        seneca.log.warn('listen', 'bad_secret', { client: req.headers['seneca-secret'], listen: listenOptions.secret })
        var badSecretOut = transportUtil.prepareResponse(seneca, {})
        badSecretOut.id = req.headers['seneca-id']
        badSecretOut.origin = req.headers['seneca-origin']
        badSecretOut.error = transportUtil.error('bad_secret', {})
        internals.sendResponse(seneca, transportUtil, res, badSecretOut, {})
        return
      }
